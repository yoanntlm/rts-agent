...
function sectorDividers(width: number, height: number) {
  const dividers: SectorInstance[] = [];
  for (const x of [Math.floor(width * 0.25), Math.floor(width * 0.5), Math.floor(width * 0.75)]) {
    dividers.push({ x, y: height / 2, width: 0.08, height: height - 2, color: "#4ECDC4" });
  }
  for (const y of [Math.floor(height * 0.33), Math.floor(height * 0.66)]) {
    dividers.push({ x: width / 2, y, width: width - 2, height: 0.08, color: "#A78BFA" });
  }
  return dividers;
}
...
export default function Tilemap({ width, height }: Props) {
  ...
  const { grass, water, workshops, details, centers, sectors, gridPositions } = useMemo(() => {
    ...
    return {
      ...
      details,
      centers,
      sectors: sectorDividers(width, height),
      gridPositions,
    };
  }, [width, height]);
  ...
  useEffect(() => {
    ...
    sectors.forEach((sector, i) => {
      MATRIX.makeScale(sector.width, sector.height, 1);
      MATRIX.setPosition(sector.x, sector.y, 0.045);
      sectorRef.current?.setMatrixAt(i, MATRIX);
      sectorRef.current?.setColorAt(i, COLOR.set(sector.color));
    });
    sectorRef.current.instanceMatrix.needsUpdate = true;
    if (sectorRef.current.instanceColor) sectorRef.current.instanceColor.needsUpdate = true;
  }, [sectors]);
  ...
  return (
    <group>
      ...
      <instancedMesh ref={sectorRef} args={[undefined, undefined, sectors.length]}>
